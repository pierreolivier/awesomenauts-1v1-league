        ...
        if (!err) {
            connection.query('SELECT * FROM `round` WHERE id_match = ? ORDER BY id', [id], function (err, rows) {
                if (!err) {
                    ...
                    var rank = 1;
                    for (var i in rows) {
                        output.push({rank: rank, score_player_1: rows[i].score_player_1, score_player_2: rows[i].score_player_2, naut_player_1: rows[i].naut_player_1, naut_player_2: rows[i].naut_player_2, map: rows[i].map, picker: rows[i].picker});
                        rank++;
                    }
