...
function getHighlightsGroup(rounds) {
    ...
    for (var i in statsHighlights) {
        statsHighlights[i].naut = getHighestOccurrence(statsHighlights[i].nauts);
...
function formatGroup(connection, rounds, cb) {
    var output = {};
    ...
    if (validator.isNumeric(id)) {
        helper.mysql.connection(function (err, connection) {
            if (!err) {
                ...
                //        if (rows.length > 0) {
                            //connection.query('SELECT *, m.id as id_match, r.id as id_round FROM `round` as r LEFT OUTER JOIN `match` as m ON m.id = r.id_match WHERE (m.id_player_1 = ' + id + ' OR id_player_2 = ' + id + ') ORDER BY r.id', function (err2, rows2) {
                            connection.query('SELECT *, p.id as id_player FROM `player` as p, `match` as m, `round` as r WHERE p.id_group = ' + id + ' AND (m.id_player_1 = p.id OR m.id_player_2 = p.id) AND r.id_match = m.id', function (err2, rows2) {
                                if (!err2) {
                                    formatGroup(connection, rows2, cb);
                                } else {
                                    cb({});
                                }
                            });
