...
function getAccomplishments(id, rounds) {
    ...
    for (var i in rounds) {
        ...
        if (gap > maxGap) {
            ...
            maxGapNaut = getNaut(id, rounds[i]);
        }
    }
...
function formatPlayer(connection, player, rounds, cb) {
    ...
    //console.log(getScore(rounds[0], id));
        ...
        helper.mysql.connection(function (err, connection) {
            if (!err) {
                connection.query('SELECT * FROM `player` WHERE id = ' + id, function (err, rows) {
                    if (!err) {
                        if (rows.length > 0) {
                            connection.query('SELECT * FROM `round` as r LEFT OUTER JOIN `match` as m ON m.id = r.id_match WHERE (m.id_player_1 = ' + id + ' OR id_player_2 = ' + id + ') ORDER BY m.date DESC', function (err2, rows2) {
                                if (!err2) {
