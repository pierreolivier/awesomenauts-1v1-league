...
function getMatches(rounds) {
    ...
    for (var i in rounds) {
        ...
        var insert = true;
        for (var j in matches[e.id_match].rounds) {
            if (matches[e.id_match].rounds[j].id == e.id_round) {
                insert = false;
                break;
            }
        }
        if (insert) {
            matches[e.id_match].rounds.push({
                id: e.id_round,
                nauts: getNautName(e.naut_player_1) + ' vs ' + getNautName(e.naut_player_2),
...
function formatGroup(connection, id, rounds, cb) {
    ...
    getGroup(connection, id, function(groupName) {
        output.group = groupName;
...
exports.group = function(id, cb) {
    if (validator.isNumeric(id)) {
        helper.mysql.connection(function (err, connection) {
            if (!err) {
               connection.query('SELECT *, p.id as id_player, r.id as id_round FROM `player` as p, `match` as m, `round` as r WHERE p.id_group = ' + id + ' AND (m.id_player_1 = p.id OR m.id_player_2 = p.id) AND r.id_match = m.id', function (err2, rows2) {
                    if (!err2) {
