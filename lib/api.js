        ...
        helper.mysql.connection(function (err, connection) {
            if (!err) {
                connection.query('SELECT * FROM `player` WHERE id = ' + id, function (err, rows) {
                    if (!err) {
                        if (rows.length > 0) {
                            connection.query('SELECT *, r.id as id_round FROM `round` as r LEFT OUTER JOIN `match` as m ON m.id = r.id_match WHERE (m.id_player_1 = ' + id + ' OR id_player_2 = ' + id + ') ORDER BY r.id DESC', function (err2, rows2) {
                                if (!err2) {
