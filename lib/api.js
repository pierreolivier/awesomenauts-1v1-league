...
exports.getNautName = function(id) {
    switch (id) {
        case 1:
            return "Froggy G";
        case 2:
...
exports.getScore = function(id, round) {
    var i = '1';
    if (round.id_player_2 == id) {
        i = '2';
    }
...
exports.getScoreAd = function(id, round) {
    var i = '2';
    if (round.id_player_2 == id) {
        i = '1';
    }
...
function getHighlights(id, rounds) {
    ...
    for (var i in rounds) {
        ...
        var win = (exports.getScore(id, rounds[i]) > exports.getScoreAd(id, rounds[i]));
...
function getAccomplishments(id, rounds) {
    ...
    for (var i in rounds) {
        var score = exports.getScore(id, rounds[i]);
        var scoreAd = exports.getScoreAd(id, rounds[i]);
        var naut = getNaut(id, rounds[i]);
    ...
    if (biggestWinningSpreeNauts.length == 0) {
        biggestWinningSpreeNauts.push(1);
    }

    console.log(biggestWinningSpreeNauts);

    output.bestImage = getNautImage(maxGapNaut);
    output.bestScore = maxGapScore;
    output.winningSpreeImage = getNautImage(getHighestOccurrence(biggestWinningSpreeNauts));
    output.winningSpreeWins = biggestWinningSpree;
...
function getMatches(rounds) {
    ...
    for (var i in rounds) {
        ...
        if (matches[e.id_match] == undefined) {
            matches[e.id_match] = {};
            matches[e.id_match].idPlayer1 = e.id_player_1;
            matches[e.id_match].idPlayer2 = e.id_player_2;
        ...
        matches[e.id_match].rounds.push({
            id: e.id_round,
            nauts: exports.getNautName(e.naut_player_1),
    ...
    for (var i in matches) {
        ...
        if (matches[i].points1 > matches[i].points2) {
            matches[i].points1 += 2;
            matches[i].idWinner = matches[i].idPlayer1;
        } else {
            matches[i].points2 += 2;
            matches[i].idWinner = matches[i].idPlayer2;
        }
...
function getGroupPlayers(connection, id, cb) {
    connection.query('SELECT *, p.id as idPlayer, p.name as playerName, g.name as group_name FROM `player` as p LEFT OUTER JOIN `group` as g ON g.id = p.id_group WHERE id_group = ' + id + ' ORDER BY p.points DESC', function (err, rows) {
        if (!err) {
            if (rows.length > 0) {
                var j = 1;
                for (var i in rows) {
...
function getNames(connection, id, matches, cb) {
    var ids = [];
    for (var i in matches) {
        if (matches[i].idPlayer1 != id) {
            ids.push(matches[i].idPlayer1);
        }
        if (matches[i].idPlayer2 != id) {
            ids.push(matches[i].idPlayer2);
        }
    }
...
function formatPlayer(connection, player, rounds, cb) {
    ...
    output.steamProfile = player.steam_profile;
    output.roundsPlayed = rounds.length;
    output.wins = player.wins;
    output.losses = player.losses;
    output.points = player.points;
    output.withWins = player.with_wins;
    output.withLosses = player.with_losses;
    ...
    output.bestImage = accomplishments.bestImage;
    output.bestScore = accomplishments.bestScore;
    output.winningSpreeImage = accomplishments.winningSpreeImage;
    output.winningSpreeWins = accomplishments.winningSpreeWins;

    output.matches = getMatches(rounds);

    output.matchesPlayed = output.matches.length;

    getGroup(connection, player.id_group, function(groupName) {
        output.idGroup = player.id_group;
        output.group = groupName;
        getNames(connection, id, output.matches, function(names) {
            if(names) {
                output.accomplishmentsEnable = (output.bestImage != 'images/0.png');
                names[id] = player.name;
                for (var i in output.matches) {
                    var match = output.matches[i];
                    match.resultPlayer1 = names[match.idPlayer1] + ' (+' + match.points1 + ')';
                    match.resultPlayer2 = names[match.idPlayer2] + ' (+' + match.points2 + ')';
                    match.winner = names[match.idWinner];
                }
                cb(output);
            } else {
                output.accomplishmentsEnable = false;
                output.noMatches = true;
                cb(output);
            }
        });
    });
...
function getAccomplishmentsGroup(rounds) {
    ...
    output.bestPlayer = maxGapPlayer;
    output.bestImage = getNautImage(maxGapNaut);
    output.bestScore = maxGapScore;
    if (output.bestPlayer == 0) {
        output.bestPlayerEnable = false;
    } else {
        output.bestPlayerEnable = true;
    }
...
function formatGroup(connection, id, rounds, cb) {
    ...
    for (var i in rounds) {
        ...
        players[round.id_player] = {id: round.id_player, name: round.name, wins: round.wins, losses: round.losses, points: round.points, withWins: round.with_wins, withLosses: round.with_losses};
    }

    for (var i in players) {
        output.points += players[i].points;
        output.withWins += players[i].withWins;
        output.withLosses += players[i].withLosses;
    }
    ...
    output.bestImage = accomplishments.bestImage;
    output.bestScore = accomplishments.bestScore;
    output.bestPlayerEnable = accomplishments.bestPlayerEnable;
    ...
    getGroupPlayers(connection, id, function(groupPlayers) {
        ...
        output.tiebreakerNaut = exports.getNautName(groupPlayers[0].tiebreaker_naut);
        ...
        getNames(connection, 0, output.matches, function(names) {
            if(names) {
                ...
                output.bestName = names[output.bestPlayer];
                for (var i in output.matches) {
                    var match = output.matches[i];
                    match.resultPlayer1 = names[match.idPlayer1] + ' (+' + match.points1 + ')';
                    match.resultPlayer2 = names[match.idPlayer2] + ' (+' + match.points2 + ')';
